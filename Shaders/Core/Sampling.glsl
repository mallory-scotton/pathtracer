...
vec3 SampleGGXVNDF(vec3 V, float ax, float ay, float r1, float r2)
{
    ...
    vec3 T1 = lensq > 0 ?
        vec3(-Vh.y, Vh.x, 0) * inversesqrt(lensq) :
        vec3(1, 0, 0);
    vec3 T2 = cross(Vh, T1);
...
void SampleSphereLight(
    in Light light,
    in vec3 scatterPos,
    inout LightSampleRec lightSample
)
{
    float r1 = rand();
    float r2 = rand();
    ...
    sampledDir =
        T * sampledDir.x + B * sampledDir.y +
        sphereCentertoSurface * sampledDir.z;
    ...
    lightSample.pdf = distSq / (light.area * 0.5 * abs(
        dot(lightSample.normal, lightSample.direction)));
}
...
)
{
    ...
    lightSample.emission = light.emission * float(numOfLights);
    lightSample.pdf = distSq / (light.area * abs(
        dot(lightSample.normal, lightSample.direction)));
}
