...
void Bvh::BuildImpl(BoundingBox const* bounds, int numbounds) {
    ...
    while (!stack.empty()) {
        ...
        if (req.numprims < 2) {
            ...
            // Start partitioning and updating extents for children at the same
            // time
            BoundingBox leftbounds, rightbounds, leftcentroid_bounds,
                rightcentroid_bounds;
            int splitidx = 0;
            ...
            // Left request
            SplitRequest leftrequest = {req.startidx, splitidx - req.startidx,
                                        &node->lc, leftbounds,
                                        leftcentroid_bounds};
            // Right request
            SplitRequest rightrequest = {
                splitidx, req.numprims - (splitidx - req.startidx), &node->rc,
                rightbounds, rightcentroid_bounds};
