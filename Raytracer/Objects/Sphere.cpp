...
void Sphere::GenerateGeometry(void)
{
    ...
    for (int stack_idx = 0; stack_idx <= m_rings; ++stack_idx)
    {
        ...
        float phi = static_cast<float>(stack_idx) * PI / static_cast<float>(m_rings);
        ...
        for (int sector_idx = 0; sector_idx <= m_segments; ++sector_idx)
        {
            ...
            float theta = static_cast<float>(sector_idx) * 2.0f * PI / static_cast<float>(m_segments);
            ...
            if (stack_idx == 0) { 
                ...
                if (length > 1e-6f) { 
                    ...
                    normal_x = 0.0f; normal_y = (y > 0) ? 1.0f : -1.0f; normal_z = 0.0f;
                }
            }

            unique_normals_temp.push_back(Vec4f(normal_x, normal_y, normal_z, 0.0f));
        }
    }
    ...
    for (int i = 0; i < m_rings; ++i) 
    {
        ...
        for (int j = 0; j < m_segments; ++j) 
        {
            ...
            if (i == 0) { 
                ...
                m_vertices.push_back(unique_vertices_temp[v_idx_top_left]);   m_normals.push_back(unique_normals_temp[v_idx_top_left]);
                m_vertices.push_back(unique_vertices_temp[v_idx_bottom_left]);  m_normals.push_back(unique_normals_temp[v_idx_bottom_left]);
                m_vertices.push_back(unique_vertices_temp[v_idx_bottom_right]); m_normals.push_back(unique_normals_temp[v_idx_bottom_right]);
            } else if (i == m_rings - 1) { 

                m_vertices.push_back(unique_vertices_temp[v_idx_top_left]);    m_normals.push_back(unique_normals_temp[v_idx_top_left]);
                m_vertices.push_back(unique_vertices_temp[v_idx_bottom_left]); m_normals.push_back(unique_normals_temp[v_idx_bottom_left]);
                m_vertices.push_back(unique_vertices_temp[v_idx_top_right]);   m_normals.push_back(unique_normals_temp[v_idx_top_right]);
            } else { 
