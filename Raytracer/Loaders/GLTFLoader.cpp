...
void LoadMeshes(Scene* scene, tinygltf::Model& gltfModel,
    ...
    for (int gltfMeshIdx = 0; gltfMeshIdx < gltfModel.meshes.size();
         gltfMeshIdx++) {
        tinygltf::Mesh gltfMesh = gltfModel.meshes[gltfMeshIdx];

        for (int gltfPrimIdx = 0; gltfPrimIdx < gltfMesh.primitives.size();
             gltfPrimIdx++) {
            ...
            tinygltf::Accessor positionAccessor =
                gltfModel.accessors[positionIndex];
            ...
            const tinygltf::Buffer& positionBuffer =
                gltfModel.buffers[positionBufferView.buffer];
            ...
            int positionStride =
                ...
                tinygltf::GetNumComponentsInType(positionAccessor.type);
            // TODO: Recheck
            ...
            if (normalIndex > -1) {
                ...
                const tinygltf::Buffer& normalBuffer =
                    gltfModel.buffers[normalBufferView.buffer];
                normalBufferAddress = normalBuffer.data.data();
                normalStride =
                    tinygltf::GetComponentSizeInBytes(
                        normalAccessor.componentType) *
                    tinygltf::GetNumComponentsInType(normalAccessor.type);
            ...
            if (uv0Index > -1) {
                ...
                const tinygltf::Buffer& uv0Buffer =
                    gltfModel.buffers[uv0BufferView.buffer];
                uv0BufferAddress = uv0Buffer.data.data();
                uv0Stride = tinygltf::GetComponentSizeInBytes(
                                uv0Accessor.componentType) *
                            tinygltf::GetNumComponentsInType(uv0Accessor.type);
            ...
            for (size_t vertexIndex = 0; vertexIndex < positionAccessor.count;
                ...
                {
                    const uint8_t* address = positionBufferAddress +
                ...
                if (normalIndex > -1) {
                    const uint8_t* address = normalBufferAddress +
                ...
                if (uv0Index > -1) {
                    const uint8_t* address =
                        uv0BufferAddress + uv0BufferView.byteOffset +
                        uv0Accessor.byteOffset + (vertexIndex * uv0Stride);
                    memcpy(&uv, address, 8);
                }
            ...
            const uint8_t* baseAddress = indexBufferAddress +
            ...
            if (indexStride == 1) {
                ...
                memcpy(quarter.data(), baseAddress,
                ...
                memcpy(indices.data(), baseAddress,
                       (indexAccessor.count * indexStride));
            }
            ...
            meshPrimMap[gltfMeshIdx].push_back(
                Primitive{sceneMeshId, sceneMatIdx});
        }
    }
}
...
void LoadMaterials(Scene* scene, tinygltf::Model& gltfModel) {
    ...
    for (size_t i = 0; i < gltfModel.materials.size(); i++) {
        ...
        // Albedo
        material.baseColor =
            Vec3f((float)pbr.baseColorFactor[0], (float)pbr.baseColorFactor[1],
        ...
        if (strcmp(gltfMaterial.alphaMode.c_str(), "OPAQUE") == 0)
        ...
        material.roughness =
            sqrtf((float)pbr.roughnessFactor);  // Repo's disney material
                                                // doesn't use squared roughness
        material.metallic = (float)pbr.metallicFactor;
        if (pbr.metallicRoughnessTexture.index > -1)
            material.metallicRoughnessTexID =
                pbr.metallicRoughnessTexture.index + sceneTexIdx;

        // Normal Map
        material.normalmapTexID =
            gltfMaterial.normalTexture.index + sceneTexIdx;

        // Emission
        material.emission = Vec3f((float)gltfMaterial.emissiveFactor[0],
                                 (float)gltfMaterial.emissiveFactor[1],
                                 (float)gltfMaterial.emissiveFactor[2]);
        if (gltfMaterial.emissiveTexture.index > -1)
            material.emissionmapTexID =
                gltfMaterial.emissiveTexture.index + sceneTexIdx;

        // KHR_materials_transmission
        if (gltfMaterial.extensions.find("KHR_materials_transmission") !=
...
void TraverseNodes(Scene* scene, tinygltf::Model& gltfModel, int nodeIdx,
                   Mat4x4f& parentMat,
                   std::map<int, std::vector<Primitive>>& meshPrimMap) {
    ...
    if (gltfNode.matrix.size() > 0) {
        ...
        if (gltfNode.rotation.size() > 0) {
            rot = Mat4x4f::QuaternionToMatrix(
                Quaternionf{static_cast<float>(gltfNode.rotation[0]),
    ...
    Mat4x4f xform = localMat * parentMat;

    // When at a leaf node, add an instance to the scene (if a mesh exists for
    // it)
    ...
    for (size_t i = 0; i < gltfNode.children.size(); i++) {
        TraverseNodes(scene, gltfModel, gltfNode.children[i], xform,
                      meshPrimMap);
    }
}

void LoadInstances(Scene* scene, tinygltf::Model& gltfModel, Mat4x4f xform,
                   std::map<int, std::vector<Primitive>>& meshPrimMap) {
    ...
    for (int rootIdx = 0; rootIdx < gltfScene.nodes.size(); rootIdx++) {
        TraverseNodes(scene, gltfModel, gltfScene.nodes[rootIdx], xform,
                      meshPrimMap);
    }
}
